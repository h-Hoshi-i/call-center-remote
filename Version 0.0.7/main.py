...
def selection_screen():
    ...
    clear_screen()
    root.geometry("100x175")

    tk.Label(root, text="Tasks", bg= "#9370DB").pack()
    tk.Button(root, text="ADMISSIONS", command=hoshi_admissions, bg="#E2D6FF").pack()
    tk.Button(root, text="RETENTION", command=retention_screen, bg="#E2D6FF").pack()
    tk.Button(root, text="EMAILS", command=gen_emails, bg="#E2D6FF").pack()
...
def retention_screen():
    clear_screen()
    root.geometry("100x150")
    tk.Label(root, text="Retention Calls", bg="#9370DB").pack()
    tk.Button(root, text="New Form", command=retention.process_form, bg="#E2D6FF").pack()
    tk.Button(root, text="Email", command=lambda: retention.process_email(saved_username), bg="#E2D6FF").pack()
...
def slate_dash():
    ...
    root.geometry("100x150")
    def sau_blocked():
        clip.copy("5 - SAU number blocked; email sent")
    ...
    tk.Button(root, text="blocked", command= sau_blocked, bg="#E2D6FF").pack()
    tk.Button(root, text="BACK", command=selection_screen, bg="#C9A3D1").pack()
...
def gen_emails():
    clear_screen()
    root.geometry("105x160")
    tk.Label(root, text="STAY HAPPY", bg= "#9370DB").pack()
    def email_select(code):
        result = morne.select_email(f"{code}", user_name=saved_username)
        clip.copy(result)

    tk.Button(root, text="base email", command=lambda:email_select("base"), bg="#E2D6FF").pack()
    tk.Button(root, text="teacher swag", command=lambda:email_select("swag"), bg="#E2D6FF").pack()
    tk.Button(root, text="College Raptor", command=lambda:email_select("raptor"), bg="#E2D6FF").pack()
    tk.Button(root, text="ADMIN COUNS", command=admin_c_select_page_1, bg="#E2D6FF").pack()
    tk.Button(root, text="BACK", command=selection_screen, bg="#C9A3D1").pack()
